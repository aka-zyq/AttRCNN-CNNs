...
class Bert_LSTM_Net (nn.Module):
    def __init__(self,input_size=768, hidden_size=100,num_layers= 1, dropout=0.2, bidirectional= False, batch_first=True, attention_size  = ATTENTION_SIZE):
        super(Bert_LSTM_Net, self).__init__()
        ...
        self.multattention = nn.MultiheadAttention(embed_dim = self.hidden_size, num_heads = MULT_HEAD,dropout = self.dropout )
...
if __name__ == '__main__':
    ...
    # get all data
    #text_all_vec, cAGR, cCON, cEXT, cNEU, cOPN, mairesse_feature = get_all_encode_label(filename = "essays_mairesse.csv", encodeAgain=False)
    ...
    for LEARNING_RATE in (0.01,0.001,0.0001,):
        for EPOCH in (30, 50, 70):
            for SENTENCE_NUM in (50, 100, 200 , 336):
                ...
                result_str = ' '
                result_str = result_str + str((EPOCH, LEARNING_RATE, BATCH_SIZE, SENTENCE_NUM, ATTENTION_SIZE, MULT_HEAD)) + "\n"
                print(EPOCH, LEARNING_RATE, BATCH_SIZE, SENTENCE_NUM)
                big_then_6 = 0
                ...
                if big_then_6 >= 2:
                    ...
                    with open('BERT_LSTM_result.txt', 'a') as f:
                        f.write(result_str + '\n\n')